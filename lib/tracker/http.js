	...
	var server = http.createServer(function (request, response) {
		request.addListener('end', function() {
			...
			try {
				...
				// console.log('peerList after  getting file');
				// console.log(file.peerList);
				
/*				var peer1 = tracker.Peer('10.0.1.160', '51413', '1074692');
				file.insertPeer(peer1);
				var peer2 = tracker.Peer('127.0.0.1', '32775', '0');
				file.insertPeer(peer2);
*/
				var peer = tracker.Peer(request.connection.remoteAddress, query["port"], query["left"]);
				console.log('tracker.Peer(request.connection.remoteAddress, query["port"], query["left"])');
				console.log(request.connection.remoteAddress);
				console.log(query["port"]);
				console.log(query["left"]);

			    // peer = file.connectPeer(query["peer_id"], peer, tracker.event(query["event"]));
				...
				// console.log('peerList before writePeers');
				// console.log(file.peerList);
				...
				console.log('peerBuffer');
				console.log(peerBuffer.toString('hex', 0, peerBuffer.length));
				response.end("e");
			} catch (failure) {
			...
			console.log('*** http.js createServer response ');
